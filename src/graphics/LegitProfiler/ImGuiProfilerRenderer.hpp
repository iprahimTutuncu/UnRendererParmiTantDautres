...
namespace ImGuiUtils {
    ...
    public:
        ...
        void RenderTimings(int graphWidth, int legendWidth, int height, int frameIndexOffset, float maxFrameTime) {
            ...
            RenderGraph(drawList, widgetPos, vec2(graphWidth, height), static_cast<size_t>(frameIndexOffset), maxFrameTime);
            RenderLegend(drawList, widgetPos + vec2(graphWidth, 0.0f), vec2(legendWidth, height), static_cast<size_t>(frameIndexOffset), maxFrameTime);
    ...
    private:
        ...
        void RenderGraph(ImDrawList *drawList, vec2 graphPos, vec2 graphSize, size_t frameIndexOffset, float maxFrameTime) {
            ...
            for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++) {
                ...
                vec2 framePos = graphPos + vec2(graphSize.x - static_cast<float>(frameWidth - 1 - (frameWidth + frameSpacing) * static_cast<int>(frameNumber)), graphSize.y - 1);
        ...
        void RenderLegend(ImDrawList *drawList, vec2 legendPos, vec2 legendSize, size_t frameIndexOffset, float maxFrameTime) {
            ...
            for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++) {
                ...
                vec2 markerRightRectMin = legendPos + vec2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth, legendSize.y - markerRigthRectMargin - (markerRightRectHeight + markerRightRectSpacing) * static_cast<float>(stat.onScreenIndex));
                vec2 markerRightRectMax = markerRightRectMin + vec2(markerRightRectWidth, -markerRightRectHeight);
                ...
                Text(drawList, markerRightRectMax + textMargin + vec2(nameOffset, 0.0f), textColor, (std::string("ms] ") + task.name).c_str());
            }
            ...
            for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
            {
              ...
              if (it != priorityEntries.end() && !it->second.isUsed)
              {
                ...
                vec2 markerRightRectMin = legendPos + vec2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth, legendSize.y - markerRigthRectMargin - (markerRightRectHeight + markerRightRectSpacing) * shownTasksCount);
                vec2 markerRightRectMax = markerRightRectMin + vec2(markerRightRectWidth, -markerRightRectHeight);
                RenderTaskMarker(drawList, markerLeftRectMin, markerLeftRectMax, markerRightRectMin, markerRightRectMax, task.color);
            ...
            /*for (size_t priorityTaskIndex = 0; priorityTaskIndex < priorityTasks.size(); priorityTaskIndex++)
            {
              ...
              vec2 taskPos = legendPos + marginSpacing + vec2(0.0f, markerHeight) + vec2(0.0f, (markerHeight + itemSpacing) * priorityTaskIndex);
              Rect(drawList, taskPos, taskPos + vec2(markerHeight, -markerHeight), task.color, true);
              Text(drawList, taskPos + textOffset, 0xffffffff, task.name.c_str());
            }*/
        }

        static void Rect(ImDrawList *drawList, vec2 minPoint, vec2 maxPoint, std::uint32_t col, bool filled = true) {
            if (filled)
        ...
        static void Text(ImDrawList *drawList, vec2 point, std::uint32_t col, const char *text) {
            drawList->AddText(ImVec2(point.x, point.y), col, text);
        }
        static void Triangle(ImDrawList *drawList, std::array<vec2, 3> points, std::uint32_t col, bool filled = true) {
            if (filled)
        ...
        static void RenderTaskMarker(ImDrawList *drawList, vec2 leftMinPoint, vec2 leftMaxPoint, vec2 rightMinPoint, vec2 rightMaxPoint, uint32_t col) {
    ...
    public:
        ...
        void Render() {
            ...
            if (static_cast<float>(graphHeight * 2 + sizeMargin + sizeMargin) < canvasSize.y) {
                ImGui::Columns(2);
